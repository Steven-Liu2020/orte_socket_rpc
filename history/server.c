...
void rpc(int cli_fd)
{
	...
	ORTEDomainInitEvents(&orteEvents);
        ...
        dm=ORTEDomainMgrCreate(cli_fd,NULL,&orteEvents,ORTE_FALSE);
	//printf("dm%x\n",dm);
	d=ORTEDomainAppCreate(cli_fd,NULL,&orteEvents,ORTE_FALSE);
	//printf("d%x\n",d);
...
void *publisherCreate(void *arg,char *topic)
{
        ...
        p=ORTEPublicationCreate(arg,topic,"answer",i2s,&pers,1,NULL,NULL,NULL);
        if(p==NULL){
